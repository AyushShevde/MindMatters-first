...
router.get("/analytics", middleware_1.requireAuth, middleware_1.requireAdmin, (req, res) => {
    try {
      ...
      `).get() || { count: 0 };
        ...
        // New bookings (last week - use signins as proxy since bookings table may not exist)
      ...
      `).get() || { count: 0 };
        ...
        // Average anxiety (use 0 if fields don't exist)
        const avgAnxietyResult = db.prepare('SELECT AVG(gad7_score) as avg FROM profiles WHERE gad7_score IS NOT NULL').get() || { avg: 0 };
        const avgAnxiety = avgAnxietyResult.avg || 0;
      ...
      `).get() || { count: 0 };
        ...
        SELECT 
          assessment_type,
          COUNT(*) as count,
          AVG(CASE WHEN json_extract(results, '$[0].score') IS NOT NULL THEN json_extract(results, '$[0].score') ELSE 0 END) as avgScore
        FROM assessments
      ...
      `).all() || [];
        ...
        SELECT 
          assessment_type,
          SUM(CASE WHEN json_extract(results, '$[0].score') < 6 THEN 1 ELSE 0 END) as low,
          SUM(CASE WHEN json_extract(results, '$[0].score') BETWEEN 6 AND 10 THEN 1 ELSE 0 END) as medium,
          SUM(CASE WHEN json_extract(results, '$[0].score') > 10 THEN 1 ELSE 0 END) as high
        FROM assessments
        GROUP BY assessment_type
      `).all() || [];
