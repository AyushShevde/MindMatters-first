...
exports.ensureDatabase = exports.getDb = void 0;
const better_sqlite3_1 = __importDefault(require("better-sqlite3"));
const path_1 = __importDefault(require("path"));
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const getDb = () => {
    const dbPath = path_1.default.join(__dirname, "..", "..", "data.sqlite");
...
const ensureDatabase = () => {
  ...
  `);
    ...
    const count = db.prepare("SELECT COUNT(*) as c FROM counselors").get();
    if (!count || count.c === 0) {
        const counselors = [
    ...
    const passwordHash = bcryptjs_1.default.hashSync("admin12345", 10);
    db.prepare("INSERT OR IGNORE INTO users (name, email, password_hash, role) VALUES (?, ?, ?, 'admin')").run(name, email, passwordHash);
    db.close();
};
