  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!res.ok) throw new Error(all?.error || 'Failed to load activities');
        const mapped: Activity[] = (all || []).map((a: any) => ({
          id: a.id,
          title: a.title,
        ...
        <Tabs value={selectedTab} onValueChange={(v) => setSelectedTab(v as any)} className="mb-10">
          ...
          <TabsContent value="all">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {/* Audio */}
              {[
                { id: 'audio-meditation', title: 'Guided Meditation for Students', description: 'Daily 10-minute calm practice', duration: '10 min' },
                ...
                { id: 'audio-exam-calm', title: 'Pre-Exam Anxiety Relief', description: 'Calm your mind before important tests', duration: '12 min' },
                { id: 'audio-study-focus', title: 'Deep Focus Music', description: 'Background sounds for concentration', duration: '45 min' },
              ...
              {/* Articles */}
              {[
                { id: 'article-exam-anxiety', title: 'Why We Feel Anxious Before Exams – Science + Tips', description: 'Understanding the science behind exam anxiety and practical coping strategies', readTime: '7 min read' },
                { id: 'article-homesickness', title: 'Homesickness in Hostels: Coping Like a Pro', description: 'Personal strategies for managing homesickness in college hostels', readTime: '6 min read' },
                { id: 'article-friends-mental-health', title: 'The Role of Friends in Mental Health', description: 'How friendships impact your mental wellbeing and how to nurture them', readTime: '8 min read' },
                { id: 'article-burnout-signs', title: 'Signs of Burnout (Beyond Tiredness)', description: 'Recognizing academic and emotional burnout before it gets worse', readTime: '5 min read' },
                { id: 'article-parents-talk', title: 'How to Talk About Mental Health with Parents', description: 'Cultural approaches to discussing mental health with family', readTime: '9 min read' },
                { id: 'article-meditation-myths', title: 'Breaking Myths: Meditation is Not Only for Adults', description: 'Debunking common misconceptions about meditation for students', readTime: '6 min read' }
              ].filter(r => r.title.toLowerCase().includes(searchQuery.toLowerCase()) || r.description.toLowerCase().includes(searchQuery.toLowerCase()))
               .map((r) => (
              ...
              {[
                ...
                { id: 'guide-peer-support', title: 'Peer Support Guide: Helping a Friend Without Judging', description: 'How to support friends going through tough times', pages: '12 pages' },
                { id: 'guide-pomodoro-routine', title: 'Pomodoro + Micro-Break Routine for Indian Students', description: 'Study technique adapted for Indian academic culture', pages: '8 pages' },
                { id: 'guide-emergency-helplines', title: 'Emergency Helplines Guide', description: 'Complete list of mental health crisis resources', pages: '4 pages' },
                { id: 'guide-counseling-booking', title: 'How to Book a Counseling Session Confidentially', description: 'Step-by-step guide to accessing professional help', pages: '6 pages' }
              ].filter(r => r.title.toLowerCase().includes(searchQuery.toLowerCase()) || r.description.toLowerCase().includes(searchQuery.toLowerCase()))
               .map((r) => (
